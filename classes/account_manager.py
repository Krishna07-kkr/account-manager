...
class RobloxAccountManager:
    ...
    def __init__(self, password=None):
        ...
        self.accounts_file = os.path.join(self.data_folder, "AccountData.enc")
        ...
        legacy_file = os.path.join(self.data_folder, "saved_accounts.json")
        if not os.path.exists(self.accounts_file) and os.path.exists(legacy_file):
            try:
                with open(legacy_file, 'r', encoding='utf-8') as f:
                    legacy_data = json.load(f)
                
                if isinstance(legacy_data, dict) and legacy_data.get('encrypted'):
                    if self.encryptor:
                        decrypted = self.encryptor.decrypt_data(legacy_data['data'])
                        if isinstance(decrypted, dict):
                            self.accounts = decrypted.get('accounts', {})
                            self.secure_settings = decrypted.get('secure_settings', {})
                        else:
                            self.accounts = {}
                    else:
                        self.accounts = {}
                    ...
                    if isinstance(legacy_data, dict):
                        if 'accounts' in legacy_data:
                            self.accounts = legacy_data.get('accounts', {})
                            self.secure_settings = legacy_data.get('secure_settings', {})
                        else:
                            self.accounts = legacy_data
                    else:
                        self.accounts = {}
                ...
                if self.encryptor:
                    ...
                    encrypted_data = self.encryptor.encrypt_data(payload)
                ...
                print(f"[ERROR] Failed to migrate legacy database: {e}")
                self.accounts = {}
                self.secure_settings = {}
        else:
    ...
    def load_accounts(self):
        with self._lock:
            if os.path.exists(self.accounts_file):
                try:
                    return self._try_load_file(self.accounts_file)
                except ValueError as ve:
                    if "Wrong password" in str(ve) or "Decryption failed" in str(ve):
                        raise
                    
                    backup_file = self.accounts_file + ".backup"
                    if os.path.exists(backup_file):
                        try:
                            restored = self._try_load_file(backup_file)
                    ...
                    raise ValueError(f"Database file is corrupted and no valid backup exists. Error: {ve}")
                except Exception as e:
                    self.secure_settings = {}
                    return {}
            self.secure_settings = {}
    ...
    def _try_load_file(self, file_path):
        ...
        if self.encryptor:
            try:
                decrypted_data = self.encryptor.decrypt_data(data_bytes)
    ...
    def _migrate_accounts(self, accounts):
        for username, account_data in accounts.items():
            if isinstance(account_data, dict):
                ...
                if 'last_use' not in account_data:
                    account_data['last_use'] = account_data.get('added_date', time.strftime('%Y-%m-%d %H:%M:%S'))
                if 'last_attempted_refresh' not in account_data:
                    account_data['last_attempted_refresh'] = ''
                if 'no_cookie_refresh' not in account_data:
                    account_data['no_cookie_refresh'] = 'false'
                if 'proxy' not in account_data:
                    account_data['proxy'] = ''
    ...
    def save_accounts(self):
        with self._lock:
            ...
            if os.path.exists(self.accounts_file):
                ...
                if os.path.exists(backup_file):
                    try:
                        last_mod = os.path.getmtime(backup_file)
                        if time.time() - last_mod < 8 * 3600:
                            should_backup = False
                    except Exception:
                        pass
                if should_backup:
            ...
            try:
                if self.encryptor:
                    ...
                    encrypted_data = json.dumps(payload, indent=2, ensure_ascii=False).encode('utf-8')
                ...
                print(f"[ERROR] Failed to save accounts atomically: {e}")
                if os.path.exists(temp_file):
                    try:
                        os.remove(temp_file)
                    except:
    ...
    def launch_roblox(self, username, game_id, private_server_id="", launcher_preference="default", job_id="", custom_launcher_path=""):
        ...
        result = RobloxAPI.launch_roblox(
            username,
            cookie,
            game_id,
            private_server_id,
        ...
        if result == "EXPIRED":
            print(f"[ERROR] Launch detected session expiration for '{username}'")
    ...
    def rotate_cookie_session(self, username):
        """
        Refresh an account's cookie by signing out of other sessions and reauthenticating.
        This forces Roblox to rotate the cookie, and we capture the new one from the response.
        """
        if username not in self.accounts:
            return False, None
        ...
        try:
            ...
            if new_cookie:
                print(f"[SUCCESS] Cookie successfully rotated for {username}!")
                self.accounts[username]['cookie'] = new_cookie
            ...
            print(f"[ERROR] Exception occurred during session cookie rotation for {username}: {e}")
            return False, None

    def set_account_proxy(self, username, proxy):
        """Set or update proxy for an account"""
        if username not in self.accounts:
            print(f"[ERROR] Account '{username}' not found")
            return False
        
        self.accounts[username]['proxy'] = proxy.strip()
        self.save_accounts()
        print(f"[SUCCESS] Proxy updated for account: {username}")
        return True

    def get_account_proxy(self, username):
        """Get proxy for a specific account"""
        if username in self.accounts:
            return self.accounts[username].get('proxy', '')
        return ''
